                                                                        ...
                                                                        unsigned int jvar_num)
{
  ...
  for (unsigned int cur_nd = 0; cur_nd < 2; cur_nd++)
  {
    ...
    neighbors = _pdmesh.getNeighbors(_nodes_ij[cur_nd]->id());
    bonds = _pdmesh.getAssocBonds(_nodes_ij[cur_nd]->id());
    for (unsigned int k = 0; k < neighbors.size(); k++)
    {
      ...
      RealGradient origin_ori_k = *node_k - *_pdmesh.nodePtr(_nodes_ij[cur_nd]->id());
...
GeneralizedPlaneStrainOffDiagOSPD::computeTempOffDiagJacobianScalar(unsigned int jvar_num)
{
  ...
  unsigned int nn_i = _pdmesh.getNeighbors(_nodes_ij[0]->id()).size();
  unsigned int nn_j = _pdmesh.getNeighbors(_nodes_ij[1]->id()).size();
